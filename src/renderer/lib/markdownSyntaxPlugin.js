...
function activateDocumentSourceOffset(
  ...
  if (target.kind === "block") {
    const atomTarget = documentSourceAtomTarget(
      view.state,
      sourceOffset,
      serializer
    );
    if (atomTarget) {
      activateMarkdownSourceAt(view, atomTarget.position, {
        atomPosition: atomTarget.position,
        sourceOffset: atomTarget.sourceOffset
      });
      return true;
    }
...
function blockSourceDocumentEdit(state, unit, source, serializer) {
  ...
  const exactSegment = documentSource?.segments.find(({ position, node }) => (
    unit.from === position
    && unit.to === position + node.nodeSize
  ));
  // Root structural units already have an exact segment boundary. Prefer it
  // over probing a rendered caret: quote/list prefixes contain multiple
  // physical offsets that intentionally collapse onto the same visual edge.
  const unitStart = exactSegment?.from
    ?? documentSourceUnitStartOffset(state, unit, serializer);
  if (
    !documentSource
    || !Number.isFinite(unitStart)
    || documentSource.fullSource.slice(unitStart, unitStart + currentSource.length) !== currentSource
  ) return null;
  return {
    fullSource: `${documentSource.fullSource.slice(0, unitStart)}${source}${
      documentSource.fullSource.slice(unitStart + currentSource.length)
    }`,
    unitStart,
    currentSource
  };
...
function replaceBlockSource(
  ...
  if (documentEdit) {
    ...
    // (for example `- item` becoming `-X item`). Keep those literal bytes
    // beside this exact document snapshot instead of letting the parsed block
    // split serialize with newly invented blank lines.
    view.dom.tetherCommittedSourceDraft = {
      doc: view.state.doc,
      markdown: documentEdit.fullSource
    };
...
export function markdownSourceDraftMarkdown(state, parser, serializer, unit, source) {
  ...
  if (unit.kind === "block") {
    const documentEdit = blockSourceDocumentEdit(state, unit, source, serializer);
    if (documentEdit) return documentEdit.fullSource;
  }
  if (unit.name === "hardbreak") {
    const node = state.doc.nodeAt(unit.from);
...
export function sourceInputHorizontalSelection(
  source,
  selectionStart,
  selectionEnd,
  selectionDirection,
  ...
  const current = sourceInputSelection(
    selectionStart,
    selectionEnd,
    selectionDirection
  );
  ...
  if (shiftKey) {
    ...
    head = sourceOffsetAfterCharacter(source, current.head, direction);
  } else if (current.anchor !== current.head) {
...
export function redirectPendingSourceTextInput(event, control) {
  if (
    !event
    ...
    || !control?.isConnected
    || typeof control.tetherApplyPendingText !== "function"
    || event.target?.closest?.(".tether-continuous-source")
  ) return false;
  event.preventDefault();
  event.stopImmediatePropagation();
  control.focus?.({ preventScroll: true });
  control.tetherApplyPendingText(event.data);
  return true;
}
...
function stripSourceMarkerFromSignature(signature, marker) {
  if (typeof signature !== "string" || !signature.includes(marker)) return signature;
  const strip = (value) => {
    if (typeof value === "string") return value.split(marker).join("");
    if (Array.isArray(value)) {
      return value
        .map(strip)
        .filter((entry) => !(
          (entry?.type === "text" && entry.value === "")
          || (entry?.type === "paragraph" && entry.children?.length === 0)
        ));
    }
    if (!value || typeof value !== "object") return value;
    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, strip(entry)])
    );
  };
  try {
    return JSON.stringify(strip(JSON.parse(signature)));
  } catch {
    return signature.split(marker).join("");
  }
}

function sanitizeMarkedSourceAttrs(attrs, marker, caretOffset) {
  return Object.fromEntries(Object.entries(attrs || {}).map(([key, value]) => {
    if (typeof value === "string") {
      return [
        key,
        key.endsWith("Signature")
          ? stripSourceMarkerFromSignature(value, marker)
          : value.split(marker).join("")
      ];
    }
    if (
      Number.isFinite(value)
      && /(SourceStart|SourceEnd|ContentStart|ContentEnd)$/.test(key)
      && value > caretOffset
    ) {
      return [key, value - marker.length];
    }
    return [key, value];
  }));
}

function sanitizeMarkedSourceDocument(node, marker, caretOffset) {
  if (node.isText) return node;
  const children = [];
  node.forEach((child) => {
    children.push(sanitizeMarkedSourceDocument(child, marker, caretOffset));
  });
  return node.type.create(
    sanitizeMarkedSourceAttrs(node.attrs, marker, caretOffset),
    Fragment.fromArray(children),
    node.marks
  );
}

function replaceMarkedDocumentSourceAtCaret(state, source, caretOffset, parser) {
  if (typeof parser !== "function") return null;
  let marker = "\uE200";
  while (source.includes(marker)) marker += "\uE201";
  const marked = normalizeEmptyMarkdownDocument(
    parser(`${source.slice(0, caretOffset)}${marker}${source.slice(caretOffset)}`),
    source
  );
  let markerPosition = null;
  marked?.descendants((node, position) => {
  ...
  if (markerPosition == null) return null;
  const sanitized = sanitizeMarkedSourceDocument(marked, marker, caretOffset);

  let transaction = state.tr.replace(
    0,
    state.doc.content.size,
    new Slice(sanitized.content, 0, 0)
  );
  for (const [name, value] of Object.entries(sanitized.attrs || {})) {
    transaction = transaction.setDocAttribute(name, value);
  }
  ...
  const selectionPosition = Math.max(
    0,
    Math.min(markerPosition, transaction.doc.content.size)
  );
...
function structuralListLineMatch(line) {
  return String(line ?? "").match(
    /^((?:[\t ]{0,3}>[\t ]?)*)([\t ]*)([-+*]|(\d+)([.)]))(?:([\t ]+)(?:\[([ xX])\]([\t ]*))?|$)/
  );
}

function structuralListContinuationPrefix(match) {
  if (!match) return null;
  const marker = match[4] == null
    ? match[3]
    : `${Number(match[4]) + 1}${match[5]}`;
  return `${match[1]}${match[2]}${marker}${match[6] || " "}${
    match[7] == null ? "" : `[ ]${match[8] || " "}`
  }`;
}
...
export function structuralEnterContinuation(source, caretOffset) {
  ...
  const lineEnd = nextNewline < 0
    ? value.length
    : nextNewline > lineStart && value[nextNewline - 1] === "\r"
      ? nextNewline - 1
      : nextNewline;
  const afterLineEnding = nextNewline < 0 ? value.length : nextNewline + 1;
  const line = value.slice(lineStart, lineEnd);
  const localCaret = caret - lineStart;
  const list = structuralListLineMatch(line);
  let prefix = null;
  let prefixLength = 0;
  if (list && localCaret >= list[0].length) {
    prefixLength = list[0].length;
    if (
      line.slice(prefixLength).trim() === ""
    ) {
      if (list[2] !== "") {
        const precedingLines = value.slice(0, lineStart).split(/\r?\n/);
        for (let index = precedingLines.length - 1; index >= 0; index -= 1) {
          const preceding = structuralListLineMatch(precedingLines[index]);
          if (
            !preceding
            || preceding[1] !== list[1]
            || preceding[2].length >= list[2].length
          ) continue;
          const outdentPrefix = structuralListContinuationPrefix(preceding);
          return {
            source: `${value.slice(0, lineStart)}${outdentPrefix}${value.slice(lineEnd)}`,
            caret: lineStart + outdentPrefix.length,
            prefix: outdentPrefix,
            exit: false,
            outdent: true,
            atLineEnd: true
          };
      ...
      if (list[1] !== "") {
        return {
          source: `${value.slice(0, lineStart)}${list[1]}${value.slice(lineEnd)}`,
          caret: lineStart + list[1].length,
          prefix: list[1],
          exit: false,
          outdent: true,
          atLineEnd: true
      ...
      return {
        source: `${value.slice(0, lineStart)}${lineEnding}${lineEnding}${
          value.slice(afterLineEnding)
        }`,
        caret: lineStart + lineEnding.length,
        prefix: "",
        exit: true,
        atLineEnd: true
      };
    }
    prefix = structuralListContinuationPrefix(list);
  } else {
    const quote = line.match(/^((?:[\t ]{0,3}>[\t ]?)+)/);
    if (quote && localCaret >= quote[0].length) {
      prefixLength = quote[0].length;
      if (
        (quote[0].match(/>/g) || []).length === 1
        && line.slice(prefixLength).trim() === ""
      ) {
        const lineEnding = sourceLineEndingAt(value, caret);
        return {
  ...
  return {
    ...
    prefix,
    exit: false,
    atLineEnd: caret + discardedSpace >= lineEnd
  };
}

function structuralEnterSourceSelectionEdit(state, sourceSelection, parser) {
  ...
  const transaction = (
    continuation.exit || continuation.atLineEnd
      ? replaceMarkedDocumentSourceAtCaret
      : replaceDocumentSourceAtCaret
  )(
    state,
    continuation.source,
    continuation.caret,
...
function renderedDocumentSourceSelection(state, serializer, selection = state?.selection) {
  ...
  if (
    ...
    const fromSelection = collapsedDocumentSourceSelection(
      state,
      serializer,
      TextSelection.create(state.doc, selection.from)
    );
    const toSelection = collapsedDocumentSourceSelection(
      state,
      serializer,
    ...
    if (
      fromSelection
      && toSelection
      && fromSelection.fullSource === toSelection.fullSource
    ) {
      const forward = selection.anchor <= selection.head;
      beforeSelection = {
        anchor: forward ? fromSelection.head : toSelection.head,
        head: forward ? toSelection.head : fromSelection.head,
        fullSource: fromSelection.fullSource,
...
function structuralEnterRangeEdit(state, parser, serializer) {
  ...
  const deletedSource = `${
    beforeSelection.fullSource.slice(0, from)
  }${beforeSelection.fullSource.slice(to)}`;
  const deletedSelection = {
    anchor: from,
    head: from,
    fullSource: deletedSource,
  ...
  if (
    ...
    const insertOffset = segment.to - removedLength;
    const lineEnding = sourceLineEndingAt(deletedSource, insertOffset);
    const source = `${
      deletedSource.slice(0, insertOffset)
    }${lineEnding}${deletedSource.slice(insertOffset)}`;
    const afterCaret = insertOffset + lineEnding.length;
    const transaction = replaceMarkedDocumentSourceAtCaret(
      state,
      source,
      afterCaret,
  ...
  const edit = structuralEnterSourceSelectionEdit(
    state,
    deletedSelection,
    parser
  );
  return edit ? { ...edit, beforeSelection } : null;
}
...
export function structuralEnterEdit(state, parser, serializer) {
  ...
  if (!selection.empty) {
    return structuralEnterRangeEdit(state, parser, serializer);
  }
  ...
  if (
    ...
    if (!segment || !Number.isFinite(contentEndOffset)) return null;

    if (caret === contentEndOffset) {
      const lineEnding = sourceLineEndingAt(beforeSelection.fullSource, caret);
      const source = `${
        beforeSelection.fullSource.slice(0, segment.to)
      }${lineEnding}${beforeSelection.fullSource.slice(segment.to)}`;
      const afterCaret = segment.to + lineEnding.length;
      const transaction = replaceMarkedDocumentSourceAtCaret(
        state,
        source,
        afterCaret,
    ...
    if (caret <= segment.from || caret > contentEndOffset) return null;
  ...
  let continuation = structuralEnterContinuation(
    beforeSelection.fullSource,
    beforeSelection.head
  );
  if (continuation) continuation.sourceCaret = beforeSelection.head;
  if (
    !continuation
    && selection.$from.parentOffset === selection.$from.parent.content.size
  ) {
    const backwardCaret = documentSourceOffsetAtPosition(
      state,
      selection.head,
      serializer,
      "backward"
    ...
    if (
      Number.isFinite(backwardCaret)
      && backwardCaret < beforeSelection.head
      && /^[\r\n]*$/.test(
        beforeSelection.fullSource.slice(backwardCaret, beforeSelection.head)
      )
    ) {
      continuation = structuralEnterContinuation(
        beforeSelection.fullSource,
        backwardCaret
      );
      if (continuation) continuation.sourceCaret = backwardCaret;
    }
  }
  return continuation
    ? structuralEnterSourceSelectionEdit(
        ...
        {
          ...beforeSelection,
          anchor: Number.isFinite(continuation.sourceCaret)
            ? continuation.sourceCaret
            : beforeSelection.anchor,
          head: Number.isFinite(continuation.sourceCaret)
            ? continuation.sourceCaret
            : beforeSelection.head
        },
        parser
...
export function documentSourceAtomTarget(state, sourceOffset, serializer) {
  if (!state?.doc || !Number.isFinite(sourceOffset) || typeof serializer !== "function") {
    return null;
  }
  let result = null;
  state.doc.descendants((node, position) => {
    if (result || !sourceAtomNames.has(node.type.name)) return !result;
    const unit = markdownAtomSyntaxAt(state, position);
    const start = unit
      ? documentSourceUnitStartOffset(state, unit, serializer)
      : null;
    const source = unit
      ? continuousMarkdownSource(state, unit, serializer)
      : "";
    if (
      Number.isFinite(start)
      && sourceOffset >= start
      && sourceOffset <= start + source.length
    ) {
      result = {
        position,
        unit,
...
export function documentPositionAtSourceOffset(state, sourceOffset, serializer) {
  ...
  const exactRenderedPosition = (position) => {
    const selection = TextSelection.create(state.doc, position);
    // Validate with the same precedence used by text input. Structural
    // paragraphs often know their exact visible source start (for example the
    // byte after `- `), while a generic block probe can also map that visual
    // edge onto an interior hidden-marker offset. Only the typing round trip
    // is safe to hand back to a rendered caret.
    const roundTrip = renderedDocumentSourceSelection(state, serializer, selection);
    return roundTrip?.head === sourceOffset ? position : null;
  };
  ...
  for (let position = unit.from; position <= unit.to; position += 1) {
    ...
    if (Number.isFinite(literalOffset)) {
      if (literalOffset === sourceOffset) {
        const exactPosition = exactRenderedPosition(position);
    ...
    if (target.segment.from + localOffset === sourceOffset) {
      const exactPosition = exactRenderedPosition(position);
      if (exactPosition != null) return exactPosition;
    }
  }
...
function continuousSourceEditor(
  ...
  // example while an arrow key is held). Seed its selection immediately so
  ...
  const finish = (commit, afterFinish = null, sync = false) => {
    ...
    const run = () => {
      ...
      // ownership record if this editor still owns it.
  ...
  editor.tetherApplyPendingText = (text) => applyClipboardEdit(text, "insertText");
  ...
  resize();
  const focusStartingSelection = () => {
    if (finished || !editor.isConnected || !shouldFocus()) return false;
    // The microtask normally claims focus first. If the user has already
    // moved or extended the seeded selection by the animation-frame fallback,
    // do not reset it to the activation range.
  ...
  // ProseMirror inserts the decoration synchronously after this constructor
  // returns. A microtask can therefore claim the caret before the next
  // keyboard event, while the animation frame below remains the layout-safe
  // fallback for a decoration that settles later.
  queueMicrotask(focusStartingSelection);
  requestAnimationFrame(() => {
    if (finished || !editor.isConnected) return;
    resize();
  ...
  return new Plugin({
    ...
    view(view) {
      ...
      const captureExactNavigation = (event) => {
        ...
        const pendingControl = activeSourceControl?.element?.isConnected
          ? activeSourceControl.element
          : null;
        if (
          pendingControl
          && !event.altKey
      ...
      const capturePendingSourceInput = (event) => {
        const pendingControl = activeSourceControl?.element?.isConnected
          ? activeSourceControl.element
          : null;
        if (!view.editable || !pendingControl) return;
        redirectPendingSourceTextInput(event, pendingControl);
      };
    ...
    props: {
      ...
      handleDOMEvents: {
        paste(view, event) {
          ...
          dispatchExactEdit(
            ...
            {
              isolatedHistory: true,
              renderedCaret: Boolean(structuralSourceSelection)
            }
          );
          return true;
        },
      ...
      decorations(state) {
        ...
        const editorDecoration = Decoration.widget(unit.from, () => continuousSourceEditor(
          ...
          publishDraft,
          () => true,
          (control, expectedElement = null) => {
            if (
              !control
              && expectedElement
              && activeSourceControl?.element !== expectedElement
            ) return;
